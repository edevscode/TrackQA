-- Any project member (including the assignee themself) could freely change
-- an issue's priority or reassign it to someone else. Restrict both to the
-- project owner, enforced in the same before-update trigger that already
-- gates status transitions so it can't be bypassed by direct table access.
...
begin
  ...
  if old.status = 'OPEN' and new.status = 'IN_PROGRESS' then
    ...
    null; -- any project member may close a passed issue
